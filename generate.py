...
from fairseq import bleu, rouge, checkpoint_utils, options, progress_bar, tasks, utils
...
def main(args):
    ...
    score = rouge.rouge(hyp_list, ref_list)
    references = [[ref.split()] for ref in ref_list]
    hypotheses = [hyp.split() for hyp in hyp_list]
    ...
    if has_target:
        ...
        print('Cumulative 3-gram: %f' % corpus_bleu(references, hypotheses, weights=[0.33, 0.33, 0.33, 0]))
        print('Cumulative 4-gram: %f' % corpus_bleu(references, hypotheses, weights=[0.25, 0.25, 0.25, 0.25]))
        # print('| Generate {} with beam={}: {}'.format(args.gen_subset, args.beam, score))
    write_to_file_hyp_ref(args.answer_output_path, hyp_list, ref_list)
    return score
